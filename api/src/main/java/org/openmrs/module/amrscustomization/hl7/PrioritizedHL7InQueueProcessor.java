 ...
 */
package org.openmrs.module.amrscustomization.hl7;
 ...
 */
@Transactional
public class PrioritizedHL7InQueueProcessor /* implements Runnable */{
    ...
    private HL7Source preferredSource = null;

    private AMRSCustomizationService service = null;
	 ...
	 */
	public PrioritizedHL7InQueueProcessor() {
	}
    ...
    private HL7Source getPreferredSource() {
        if (preferredSource == null) {
            String sourceName = Context.getAdministrationService().getGlobalProperty(AMRSCustomizationConstants.GP_PREFERRED_HL7_SOURCE, null);
            preferredSource = StringUtils.hasText(sourceName) ? Context.getHL7Service().getHL7SourceByName(sourceName) : null;
        }
        return preferredSource;
    }

    private AMRSCustomizationService getService() {
        if (service == null)
            service = Context.getService(AMRSCustomizationService.class);
        return service;
    }
